				...
				<div className="bg-gray-50 min-h-screen py-10">
					<div className="max-w-4xl mx-auto bg-white shadow-md rounded-md p-6">
						...
						<div className="flex justify-between items-center border-b pb-4">
							<h1 className="flex items-center text-xl font-bold"><CircleCheck className='mr-2 text-green-500'/>Request Confirmed</h1>
							
						</div>
						<div className="py-4 [&_p]:my-4 [&_b]:text-lg [&_b]:underline">
							<p>
								Your request for <b>{reqData.req_item_title}</b> - <b>{reqData.req_item_id}</b> has been accepted!
							</p>
						...
						<div className="flex justify-center items-center pt-4">
							<Link className="mx-1" href={reqData.req_back_to_record}><Button>Back to Record</Button></Link>
							{navigations.map((item) => (
								<Link className="mx-1" href={item.url}><Button>{item.title}</Button></Link>
							))}
						</div>
						</div>
					</div>
