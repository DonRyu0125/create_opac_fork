		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`/scripts/mwimain.dll/${getLanguageID()}/CLIENT_VIEW/WEB_CLIENT/PATRON_INT_ID%20${getPatronID()}?COMMANDSEARCH`,
			...
			<section>
				<div className="bg-gray-50 min-h-screen py-10">
					<div className="max-w-4xl mx-auto bg-white shadow-md rounded-md p-6">
						...
						<div className="py-4">
							<form method="post" className="m-0" action={`${formActionSaveRecord}&RETURN_URL=[OPAC_REPROD]reproductionconfirmed.html`}>
								...
								<Input type="hidden" name="REQ_ITEM_ID" className="w-full p-2 border rounded mb-4" value={itemid} readOnly />
